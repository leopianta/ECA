...
Class petiDao{
    ...
    public function remover($peti)
    {
        ...
        try{
            ...
            $statement->bindValue(":id", $peti->getIdTbPeti());
    ...
    public function salvar($peti)
    {
        ...
        try{
            if ($peti->getIdTbPeti() != "") {
                $statement = $pdo->prepare("UPDATE tb_peti SET tb_situacao_beneficiario=:tb_situacao_beneficiario,tb_valor_parcela=:tb_valor_parcela,str_month=:str_month, 
 str_year=:str_year, tb_beneficiaries_id_beneficiaries=:tb_beneficiaries_id_beneficiaries, 
 tb_city_id_city=:tb_city_id_city WHERE id_tb_peti= :id;");
                $statement->bindValue(":id", $peti->getIdTbPeti());
            } else{
                $statement = $pdo->prepare("INSERT INTO tb_peti (tb_situacao_beneficiario, tb_valor_parcela, str_month, str_year, db_value, tb_beneficiaries_id_beneficiaries, tb_city_id_city ) VALUES (:tb_situacao_beneficiario, :tb_valor_parcela:str_month, :str_year, :db_value, :tb_beneficiaries_id_beneficiaries, :tb_city_id_city)");
            }

            $statement->bindValue(":tb_situacao_beneficiario", $peti->getTbSituacaoBeneficiario());
            $statement->bindValue(":tb_valor_parcela", $peti->getTbValorParcela());
